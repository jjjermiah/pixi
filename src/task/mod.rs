...
impl Task {
    ...
    pub fn as_task_description_combo(&self) -> (TaskName, String) {
        match self {
            Task::Plain(_) => (TaskName("".to_string()), "".to_string()),
            Task::Custom(custom) => (
                TaskName(custom.cmd.as_single().into_owned()),
                "".to_string(),
            ),
            Task::Execute(execute) => {
                ...
                (TaskName(execute.cmd.as_single().into_owned()), description)
            }
            Task::Alias(alias) => (
                TaskName(alias.depends_on.iter().map(|t| t.as_str()).join(",")),
                "".to_string(),
            ),
        }
    }
